 ...
 * underling field apply for the individual array items not on this
 * property.</p>
 *
 * @author ytoh
 */
public class DefaultArrayProperty extends AbstractProperty<Object> {
    ...
    /**
     * Creates an instance of <code>DefaultArrayProperty</code> wrapping an
     * array
     * {@link Field}.
     *
     * @param annotation     <code>Property</code> annotation defining this
     *                       property
     * @param field          underlying <code>Field</code>
     * @param arrayReference a reference to the array to wrap
     * @param validator      {@link Validator} instance used to validate array
     *                       items
     */
    public DefaultArrayProperty(
            cz.cvut.fit.alg.params.annotations.Property annotation, Field field,
     ...
     * is changed. (e.g. editorComponents and rendererComponents are
     * recreated)</p>
     *
    ...
    public void setValue(Object value, boolean propagate) {
        if (this.arrayProperties == null || arrayReference != value) {
            ...
            for (int i = 0, size = Array.getLength(arrayReference); i < size; i++) {
                ...
                ArrayProperty p = new ArrayProperty(field, sandbox,
                                                    arrayReference, validator, i);
    ...
    private Component createEditorComponent() {
        ...
        for (int i = 0, size = arrayProperties.size(); i < size; i++) {
            ...
            Component e = editor.getEditorComponent(arrayProperties.get(i),
                                                    editorAnnotation, context);
            horizontal.add(
                    layout.createSequentialGroup().add(label).addPreferredGap(
                            LayoutStyle.RELATED).add(e).addContainerGap());
            vertical.add(layout.createParallelGroup(GroupLayout.BASELINE).add(
                    label).add(e));
    ...
    private Component createRendererComponent() {
        ...
        for (int i = 0, size = arrayProperties.size(); i < size; i++) {
            JLabel label = new JLabel(String.valueOf(i));
            Component r = renderer.getRendererComponent(arrayProperties.get(i),
                                                        rendererAnnotation);
            horizontal.add(
                    layout.createSequentialGroup().add(label).addPreferredGap(
                            LayoutStyle.RELATED).add(r).addContainerGap());
            vertical.add(layout.createParallelGroup(GroupLayout.BASELINE).add(
                    label).add(r));
    ...
    private static final class ArrayProperty extends AbstractProperty<Object> {
        ...
        private ArrayProperty(Field field, Object sandbox, Object arrayReference,
                Validator validator, int index) {
        ...
        public void configure(Configuration provider) {
            if (Boolean.class.equals(getFieldType()) || Boolean.TYPE.equals(
                    getFieldType())) {
                setValue(provider.getBoolean(getFieldName()));
            } else if (Byte.class.equals(getFieldType()) || Byte.TYPE.equals(
                    getFieldType())) {
                setValue(provider.getByte(getFieldName()));
            } else if (Double.class.equals(getFieldType()) || Double.TYPE.equals(
                    getFieldType())) {
                setValue(provider.getDouble(getFieldName()));
            } else if (Float.class.equals(getFieldType()) || Float.TYPE.equals(
                    getFieldType())) {
                setValue(provider.getFloat(getFieldName()));
            } else if (Integer.class.equals(getFieldType()) || Integer.TYPE.equals(
                    getFieldType())) {
                setValue(provider.getInt(getFieldName()));
            } else if (Long.class.equals(getFieldType()) || Long.TYPE.equals(
                    getFieldType())) {
                setValue(provider.getLong(getFieldName()));
            } else if (Short.class.equals(getFieldType()) || Short.TYPE.equals(
                    getFieldType())) {
